...
<head>
    ...
    <title>Editar {{ $project->name }} | command.flow</title>
    <style>
        ...
        .brand, .eyebrow, .safe-note { color: #9aeaf5; font: .72rem "Cascadia Code", Consolas, monospace; letter-spacing: .1em; text-transform: uppercase; }
        ...
        .safe-note { color: #75efbd; letter-spacing: .06em; }
...
<body>
    <div class="shell">
        <header><a class="brand" href="{{ route('projects.show', $project) }}">command.flow</a></header>
        <main>
            ...
            <form method="post" action="{{ route('projects.update', $project) }}">
                @csrf
                @method('PUT')
                <p class="safe-note">No ejecuta Git: solo actualiza datos locales de la aplicación.</p>
                <label>Nombre del proyecto<input name="name" value="{{ old('name', $project->name) }}" required autofocus></label>
                @error('name')<span class="error">{{ $message }}</span>@enderror
                <label>Ruta del repositorio<input name="repo_path" value="{{ old('repo_path', $project->repo_path) }}" required></label>
                @error('repo_path')<span class="error">{{ $message }}</span>@enderror
                <label>Contexto reutilizable del proyecto<textarea name="rules">{{ old('rules', $project->rules) }}</textarea></label>
