...
@section('content')
    <section class="panel command-hero">
        ...
        <p>La conexión de Gmail se inicia desde Integraciones. Usá las opciones avanzadas solo para registrar o cambiar las credenciales de Google OAuth.</p>
    </section>
    <form method="post" action="{{ route('settings.integrations.update') }}">
        ...
        <section class="panel">
            <details class="technical-details">
                <summary>Configuración avanzada de Google OAuth</summary>
                <p class="panel-copy">Necesaria solo antes de la primera conexión o para cambiar la aplicación de Google Cloud. Los secretos se guardan en SQLite cifrados con `APP_KEY`; los valores faltantes usan el fallback opcional de `.env`.</p>
                <div class="form-grid">
                    ...
                    <p class="wide-field panel-copy">Se aceptan <code>/integrations/gmail/callback</code> y <code>/integrations/gmail/oauth/callback</code>. Registrá en Google Cloud la misma URI que configurás acá.</p>
                    <p class="wide-field panel-copy">Permisos solicitados: {{ implode(', ', $google['scopes']) }}. Consultá <code>docs/gmail-oauth.md</code> para registrar la URI.</p>
                </div>
            </details>
        </section>
        <section class="panel">
